...
class InitStoreInput(BaseModel):
    name: str = Field(description="Unique identifier for the store")
    type: Literal["list", "text", "dict"] = Field(
...
class StoreAppendInput(BaseModel):
    store: str = Field(description="Name of the store to append to")
    value: str = Field(description="Value to add to the store (resolved at runtime)")
    key: Optional[str] = Field(default=None, description="Key for dict stores (required for dict type, ignored for list/text)")
    separator: str = Field(default="\n", description="Separator for text stores when concatenating (default: newline)")


class StoreReadInput(BaseModel):
    store: str = Field(description="Name of the store to read from")
    key: Optional[str] = Field(default=None, description="For dict stores: specific key to retrieve (optional, returns full dict if not provided)")


class BuildTextInput(BaseModel):
    source: str = Field(description="Store name or variable reference (resolved at runtime)")
    mode: Literal["join"] = Field(default="join", description="Formatting mode: 'join' concatenates items with separator")
    template: Optional[str] = Field(default=None, description="Jinja2 template for custom formatting. Receives data as 'items' variable. If provided, overrides mode.")
...
class InitStore(Tool):
    ...
    def __call__(self, inputs: InitStoreInput):
...
class StoreAppend(Tool):
    ...
    def __call__(self, inputs: StoreAppendInput):
        # Tool is stateless - returns declaration of intent
...
class StoreRead(Tool):
    ...
    def __call__(self, inputs: StoreReadInput):
        # Tool is stateless - returns declaration of intent
...
class BuildText(Tool):
    ...
    def _returns_schema(self) -> dict:
        return {
            "type": "object",
            "properties": {
                "text": {"type": "string", "description": "The formatted text output"}
            },
            "required": ["text"]
        }

    def __call__(self, inputs: BuildTextInput):
