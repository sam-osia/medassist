...
RESULTS_PATH = '../../dataset/sdoh_parsed.json'
...
for sheet in sheet_names:
    ...
    df = df[['note_id', 'note_text', 'result_classification', 'result_span', 'result_reasoning', 'mrn', 'csn', 'note_csn_id', 'note_type', 'note_create_time']]
...
def build_patient_blob(mrn):
    ...
    # Create patient-level data with demographics
    patient_dict = {
        'mrn': int(mrn) if not pd.isna(mrn) else None,
    ...
    for csn in unique_csns:
        ...
        for _, row in encounter_notes.iterrows():
            ...
            # Clean the note text to normalize whitespace
...
sdoh_list = ['Language barrier', 'Financial strain', 'Social isolation', 'Housing insecurity', 'Depression', 'Addiction', 'Food insecurity', 'Transportation', 'Health literacy']
...
def process_sdoh_results():
    ...
    for sheet_name in sheet_names:
        ...
        for _, note_row in sheet_df.iterrows():
            ...
            if pd.isna(mrn) or pd.isna(csn):
                continue
                
            # Find patient results
            if mrn in patient_results:
                # Find the encounter
                encounter_found = False
                for encounter in patient_results[mrn]['encounters']:
                    if encounter['csn'] == csn:
                        ...
                        if result["flag_detected"]:
                            ...
                            print(f"    ✓ Flag {sheet_name} detected for MRN {mrn}, CSN {csn}, Note {note_id}")
                        break
                
                if not encounter_found:
                    print(f"    ⚠ Encounter CSN {csn} not found for MRN {mrn}")
            else:
                print(f"    ⚠ Patient MRN {mrn} not found in results")
