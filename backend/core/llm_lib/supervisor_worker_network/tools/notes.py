...
class SummarizePatientNote(Tool):
    ...
    def __call__(self, inputs: SummarizePatientNoteInput) -> str:
        ...
        return call_gpt(messages=messages, model_name="GPT 4o", system_message=system_prompt, count_tokens=True)
    ...
    def __call__(self, inputs: AnalyzeNoteWithSpanAndReasonInput) -> AnalyzeNoteWithSpanAndReasonOutput:
        ...
        try:
            response = call_gpt_parsed(
                ...
                json_schema=AnalyzeNoteWithSpanAndReasonOutput,
                model_name="GPT 4o",
                system_message=system_prompt
            )
            return response
...
class KeywordCount(Tool):
    ...
    """
        ...
        try:
            response = call_gpt_parsed(
                input_text=user_prompt,
                json_schema=KeywordCountOutput,
                model_name="GPT 4o",
                system_message=system_prompt
            )
            return response
...
"""
        ...
        try:
            response = call_gpt_parsed(
                input_text=user_prompt,
                json_schema=IdentifyFlagOutput,
                model_name="GPT 4o",
                system_message=system_prompt
            )
            return response
